...
SUBJECTS = ['Ambient', 'House', 'Men', 'Pets', 'Toys', 'Vehicle', 'Women']
...
def parse_function(filename, label, factor, train=False):
  ...
  img = tf.image.resize(img_decoded, [227, 227])
...
class DataGenerator(object):
  ...
  def __init__(self, data_dir, dataset, batch_size=1, num_threads=1, train_shuffle=False, buffer_size=10000):
    ...
    self.iterator = tf.compat.v1.data.Iterator.from_structure(tf.compat.v1.data.get_output_types(self.train_set),
                                                    tf.compat.v1.data.get_output_shapes(self.train_set))
    ...
    for subject in SUBJECTS:
      test_set, test_size = self._build_test_set(os.path.join(data_dir, dataset, 'val_{}.txt'.format(subject)))
      self.test_sets[subject] = test_set
      self.test_sizes[subject] = test_size
      self.test_inits[subject] = self.iterator.make_initializer(test_set)
      self.test_batches_per_epoch[subject] = int(np.ceil(test_size / batch_size))

  def load_train_set(self, sess):
    sess.run(self.train_init_opt)

  def load_test_set(self, sess, subject):
    sess.run(self.test_inits[subject])
  ...
  def _parse_factor(self, img_path):
    ...
    if self.dataset == 'children':
      factor = '{}_{}'.format(tokens[0], tokens[1])
    else:
