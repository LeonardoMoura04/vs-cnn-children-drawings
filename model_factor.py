...
class FVS_CNN(object):
  ...
  def _build_graph(self):
    self.x = tf.compat.v1.placeholder(tf.float32, [None, 227, 227, 3])
    self.y = tf.compat.v1.placeholder(tf.float32, [None, 2])

    if self.factor_layer == 'conv1':
      self.factor_weights = tf.compat.v1.placeholder(tf.float32, shape=[11, 11, 3, self.num_factors])
    elif self.factor_layer == 'conv3':
      self.factor_weights = tf.compat.v1.placeholder(tf.float32, shape=[3, 3, 128, self.num_factors])
    elif self.factor_layer == 'conv5':
      self.factor_weights = tf.compat.v1.placeholder(tf.float32, shape=[3, 3, 192, self.num_factors])
    elif self.factor_layer == 'fc7':
      self.factor_weights = tf.compat.v1.placeholder(tf.float32, shape=[4096, self.num_factors])

    self.factor_biases = tf.compat.v1.placeholder(tf.float32, shape=[self.num_factors])
    ...
    with tf.compat.v1.variable_scope('{}_factor'.format(self.factor_layer), reuse=True):
      self.assign_factor = tf.group(tf.compat.v1.get_variable('weights').assign(self.factor_weights),
                                    tf.compat.v1.get_variable('biases').assign(self.factor_biases))
  ...
  def update_factor_weights(self, session, factor_ids):
    ...
    with tf.compat.v1.variable_scope('{}_factor'.format(self.factor_layer), reuse=True):
      self.factor_weight_dict[factor_id], self.factor_bias_dict[factor_id] = \
        session.run([tf.compat.v1.get_variable('weights'), tf.compat.v1.get_variable('biases')])
      ...
      if op_name not in self.skip_layers:
        ...
        with tf.compat.v1.variable_scope(op_name, reuse=True):
          for data in self.weights_dict[op_name]:
            if len(data.shape) == 1:
              var = tf.compat.v1.get_variable('biases', trainable=True)
              session.run(var.assign(data))
            else:
              var = tf.compat.v1.get_variable('weights', trainable=True)
              session.run(var.assign(data))

    print('Loading weights for layer: {}_shared'.format(self.factor_layer))
    if self.factor_layer.startswith('conv'):
      with tf.compat.v1.variable_scope('{}_shared'.format(self.factor_layer), reuse=True):
      ...
      with tf.compat.v1.variable_scope('fc7_shared', reuse=True):
