...
tf.compat.v1.flags.DEFINE_string("factor_layer", "conv1",
                       """Name of layer to place the factors [conv1, conv3, conv5, fc7] (default: fc7)""")
tf.compat.v1.flags.DEFINE_integer("num_factors", 16,
                        """Number of specific neurons for children (default: 16)""")
tf.compat.v1.flags.DEFINE_integer("num_checkpoints", 1,
...
tf.compat.v1.flags.DEFINE_integer("display_step", 1000,
...
if tf.io.gfile.exists(weight_dir):
  tf.compat.v1.gfile.DeleteRecursively(weight_dir)
tf.io.gfile.makedirs(weight_dir)
...
def train_fn(loss, generator):
  ...
  opt1 = tf.compat.v1.train.MomentumOptimizer(learning_rate, momentum=0.5)
  opt2 = tf.compat.v1.train.MomentumOptimizer(10 * learning_rate, momentum=0.5)
...
def test(sess, model, generator, result_file):
  ...
  test_sizes = np.asarray(test_sizes, dtype=int)
...
def save_model(sess, model, saver, epoch, factor_id_map):
  ...
  for factor_id in model.factor_weight_dict.keys():
    ...
    if not tf.io.gfile.exists(factor_weight_path):
      tf.io.gfile.makedirs(factor_weight_path)
